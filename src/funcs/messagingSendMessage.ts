 ...
 * Send Message
 */
export function messagingSendMessage(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MessagingSendMessageRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.MessagingMessageSendRequestDto, {
    explode: true,
  });

  const path = pathToFunc("/unified/messaging/messages")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.MessagingSendMessageResponse$inboundSchema, {
      key: "CreateResult",
    }),

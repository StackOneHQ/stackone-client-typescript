 ...
 */
export function screeningWebhookScreeningResult(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ScreeningWebhookScreeningResultRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.ScreeningResultWebhook, {
    explode: true,
  });

  const path = pathToFunc("/unified/screening/results/webhook")();
  ...
  >(
    M.json(
      200,
      operations.ScreeningWebhookScreeningResultResponse$inboundSchema,
      { key: "ScreeningResultWebhook" },
    ),

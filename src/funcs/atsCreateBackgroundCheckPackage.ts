 ...
 */
export async function atsCreateBackgroundCheckPackage(
  client: StackOneCore,
  request: operations.AtsCreateBackgroundCheckPackageRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.AtsCreateBackgroundCheckPackageResponse,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AtsCreateBackgroundCheckPackageRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON(
    "body",
    payload.AtsCreateBackgroundCheckPackagesRequestDto,
    { explode: true },
  );
  ...
  >(
    M.json(
      201,
      operations.AtsCreateBackgroundCheckPackageResponse$inboundSchema,
      { key: "CreateResult" },
    ),

 ...
 * Get Conversation
 */
export function messagingGetConversation(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MessagingGetConversationRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/unified/messaging/conversations/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.MessagingGetConversationResponse$inboundSchema, {
      key: "MessagingConversationResult",
    }),

 ...
 * Create Conversation
 */
export function messagingCreateConversation(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MessagingCreateConversationRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON(
    "body",
    payload.MessagingCreateConversationRequestDto,
    { explode: true },
  );

  const path = pathToFunc("/unified/messaging/conversations")();
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, operations.MessagingCreateConversationResponse$inboundSchema, {
      key: "CreateResult",
    }),

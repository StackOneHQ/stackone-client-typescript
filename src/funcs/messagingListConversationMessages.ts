 ...
 */
export function messagingListConversationMessages(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MessagingListConversationMessagesRequest$outboundSchema.parse(
        value,
      ),
  ...
  >(
    M.json(
      200,
      operations.MessagingListConversationMessagesResponse$inboundSchema,
      { key: "MessagingMessagesPaginated" },
    ),
  ...
  const nextFunc = (
    ...
    const nextVal = () =>
      messagingListConversationMessages(
        client,
        {
          ...request,
          next: nextCursor,

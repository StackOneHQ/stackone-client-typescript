 ...
 */
export function hrisUpdateEmployeeTask(
  client: StackOneCore,
  request: operations.HrisUpdateEmployeeTaskRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.HrisUpdateEmployeeTaskResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.HrisUpdateEmployeeTaskRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.UpdateTaskRequestDto, {
    explode: true,
  });
  ...
  const path = pathToFunc("/unified/hris/employees/{id}/tasks/{subResourceId}")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.HrisUpdateEmployeeTaskResponse$inboundSchema, {
      key: "UpdateResult",
    }),

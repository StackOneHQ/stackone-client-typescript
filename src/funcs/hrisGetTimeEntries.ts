 ...
 * Get Time Entry
 */
export async function hrisGetTimeEntries(
  client: StackOneCore,
  request: operations.HrisGetTimeEntriesRequest,
  options?: RequestOptions,
): Promise<
  Result<
    operations.HrisGetTimeEntriesResponse,
    | SDKError
    | SDKValidationError
    | UnexpectedClientError
    | InvalidRequestError
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.HrisGetTimeEntriesRequest$outboundSchema.parse(value),
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(200, operations.HrisGetTimeEntriesResponse$inboundSchema, {
      key: "TimeEntriesResult",
    }),
    M.fail(408),

 ...
 */
export async function hrisListEmployeeTimeOffBalances(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.HrisListEmployeeTimeOffBalancesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/unified/hris/employees/{id}/time_off_balances")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.HrisListEmployeeTimeOffBalancesResponse$inboundSchema,
      { key: "TimeOffBalancesPaginated" },
    ),
    M.fail(408),

 ...
 * Get File
 */
export function documentsGetFile(
  client: StackOneCore,
  request: operations.DocumentsGetFileRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.DocumentsGetFileResponse,
...
> {
  const parsed = safeParse(
    request,
    (value) => operations.DocumentsGetFileRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/unified/documents/files/{id}")(pathParams);
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.DocumentsGetFileResponse$inboundSchema, {
      key: "FileResult",
    }),

 ...
 * List Attachments
 */
export function messagingListAttachments(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.MessagingListAttachmentsRequest$outboundSchema.parse(value),
  ...
  const path = pathToFunc("/unified/messaging/messages/{id}/attachments")(
    pathParams,
  );
  ...
  const [result, raw] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.MessagingListAttachmentsResponse$inboundSchema, {
      key: "MessagingAttachmentsPaginated",
    }),

 ...
 */
export async function marketingUpdateOmniChannelTemplate(
  client$: StackOneCore,
...
> {
  ...
  const parsed$ = schemas$.safeParse(
    input$,
    (value$) =>
      operations.MarketingUpdateOmniChannelTemplateRequest$outboundSchema.parse(
        value$,
      ),
  ...
  const body$ = encodeJSON$(
    "body",
    payload$.MarketingCreateTemplateRequestDto,
    { explode: true },
  );
  ...
  const path$ = pathToFunc("/unified/marketing/templates/omni_channel/{id}")(
    pathParams$,
  );
  ...
  >(
    m$.json(
      200,
      operations.MarketingUpdateOmniChannelTemplateResponse$inboundSchema,
      { key: "CreateResult" },
    ),

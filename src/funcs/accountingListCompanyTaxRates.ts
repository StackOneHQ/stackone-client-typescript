 ...
 * List Tax Rates
 */
export function accountingListCompanyTaxRates(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AccountingListCompanyTaxRatesRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/unified/accounting/companies/{id}/tax_rates")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.AccountingListCompanyTaxRatesResponse$inboundSchema,
      { key: "AccountingTaxRatesPaginated" },
    ),

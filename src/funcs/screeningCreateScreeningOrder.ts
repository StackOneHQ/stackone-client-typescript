 ...
 */
export function screeningCreateScreeningOrder(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.ScreeningCreateScreeningOrderRequest$outboundSchema.parse(
        value,
      ),
  ...
  const body = encodeJSON("body", payload.ScreeningCreateOrderRequestDto, {
    explode: true,
  });

  const path = pathToFunc("/unified/screening/orders")();
  ...
  >(
    M.json(
      201,
      operations.ScreeningCreateScreeningOrderResponse$inboundSchema,
      { key: "CreateResult" },
    ),

 ...
 * List Journals
 */
export function accountingListCompanyJournals(
  client: StackOneCore,
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.AccountingListCompanyJournalsRequest$outboundSchema.parse(
        value,
      ),
  ...
  const path = pathToFunc("/unified/accounting/companies/{id}/journals")(
    pathParams,
  );
  ...
  >(
    M.json(
      200,
      operations.AccountingListCompanyJournalsResponse$inboundSchema,
      { key: "AccountingJournalsPaginated" },
    ),
